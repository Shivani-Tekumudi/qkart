...
const Checkout = () => {
  ...
  const [newaddressload, setnewaddload]=useState("")
  ...
  const addAddress = async (token, newAddress) => {
    if(newAddress.value.length<20){
     return   enqueueSnackbar("Address should be greater than 20 characters", { variant: "error" });
    }
    try {
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      console.log("token", token,"addressId",addressId)
  ...
  const validateRequest = (items, addresses) => {
    if(!addresses){
     ...
     else if (getTotalCartValue(items)> localStorage.getItem("balance")){
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
        {
          variant: "warning",
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              addresses.all.map((address)=>{
                // select not selected class pettali
                return(
                <Box key={address._id} 
            ...
            <Button  onClick={() => {validateRequest(items, addresses)}} startIcon={<CreditCard />} variant="contained">
              PLACE ORDER
            </Button>
          </Box>
        </Grid>
